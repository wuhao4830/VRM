...
public class ArticleItemWriter implements ItemWriter<Article> {
	private static final Logger logger = LoggerFactory.getLogger(ArticleItemWriter.class);
	private ArticleWriteDao articleWriteDao;

	public ArticleWriteDao getArticleWriteDao() {
		return articleWriteDao;
	}
	/**
	 * Public setter for the {@link ArticleWriteDao}.
	 * @param articleWriteDao the {@link ArticleWriteDao} to set
	 */


	public void setArticleWriteDao(ArticleWriteDao articleWriteDao) {
		this.articleWriteDao = articleWriteDao;
	}



	public void write(List<? extends Article> articleCredits)  {
		for (Article article : articleCredits) {
			try {
				articleWriteDao.writeArticle(article);
			}catch (Exception e){
