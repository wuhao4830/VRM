...
public class OldRowMapper implements RowMapper<OldSkukey> {

    private static final Logger logger = LoggerFactory.getLogger(OldRowMapper.class);
    private static final String SKUID="sku_id";
    private static final String MARKETID="market_id";
	public OldSkukey mapRow(ResultSet rs, int rowNum) throws SQLException {

        OldSkukey oldSkukey=new OldSkukey();
        try {
            oldSkukey.setSkuId(rs.getString(SKUID));
            oldSkukey.setMarketId(rs.getInt(MARKETID));

        }catch (Exception e){
            logger.error(String.format("site error:%s", e.toString()));
            //发消息
            IssueInfo info=new IssueInfo();
            String desc="error:"+e.toString()+",value:"+ JSON.toJSON(oldSkukey).toString();
