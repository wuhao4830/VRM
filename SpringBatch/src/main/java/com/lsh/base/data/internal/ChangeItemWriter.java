 ...
 * @author wuhao
 */
public class ChangeItemWriter implements ItemWriter<Status> {

	private static final Logger logger = LoggerFactory.getLogger(ChangeItemWriter.class);
	...
	public void write(List<? extends Status> statuses)  {
		for (Status status : statuses) {
			try {
				genVals.saveIsChangeVal(status.getSkuId()+status.getMarketId());
			}catch (Exception e){
